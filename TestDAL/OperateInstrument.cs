...
namespace TestDAL
{
    public class OperateInstrument
    {
        ...
        private SensitivityList sensitivityList;
        private ModulationList modulationListL, modulationListM, modulationListH;
        ...
        public void InitInstr()
        {
            try
            {
                ...
                if (data.OP)
                {
                    //instrument.VisaWrite("SCPTTSTGP 3,STDTSTS,ON");
                ...
                if (data.EVM)
                {
                    ...
                    instrument.VisaWrite(string.Format("ECMCFG 3,DHXPKT,3,OFF"));
     ...
     , data.Sen_TX_Power));
        ...
        public TestData Open_MT8852(TestData item)
        {
            try
            {
                modulationListL = new ModulationList();
                modulationListM = new ModulationList();
                modulationListH = new ModulationList();
                sensitivityList = new SensitivityList();
        ...
        public TestData GetSingleSensitivity(TestData item)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                {
                    string status = retureVal.Split(',')[5];
                    //if (status == "PASS")
                    //{
        ...
        public TestData GetCarrierMod992M(TestData item)
        {
            try
            {
                double evm = 0;
                try
                {
                    if (item.Other.Split(':')[1] == data.Low_Freq)
                    {
                        evm = double.Parse(Levm.Split(',')[5]);
                    }
                    else if (item.Other.Split(':')[1] == data.Mod_Freq)
                    {
                        evm = double.Parse(Mevm.Split(',')[5]);
                    }
                    else if (item.Other.Split(':')[1] == data.Hi_Freq)
                    {
                        evm = double.Parse(Hevm.Split(',')[5]);
                    }
        ...
        public TestData GetCarrierMod993M(TestData item)
        {
            try
            {
                double evm = 0;
                try
                {
                    if (item.Other.Split(':')[1] == data.Low_Freq)
                    {
                        evm = double.Parse(Levm.Split(',')[14]);
                    }
                    else if (item.Other.Split(':')[1] == data.Mod_Freq)
                    {
                        evm = double.Parse(Mevm.Split(',')[14]);
                    }
                    else if (item.Other.Split(':')[1] == data.Hi_Freq)
                    {
                        evm = double.Parse(Hevm.Split(',')[14]);
                    }
...
//        }
        ...
        public TestData GetModulationindexF1Avg(TestData item)
        {
            string retureVal = string.Empty;
            double avgPower = 0;
            instrument.Cls();
            ...
            if (item.Other.Split(':')[1] == data.Low_Freq)
            {
                retureVal = instrument.VisaQuery("XRESULT MI,HOPOFFL");
                if (retureVal.Contains("XMI"))
                {
                    avgPower = (double.Parse(retureVal.Split(',')[4]) / 1000)
               + double.Parse(item.FillValue);
                    modulationListL.F2 = (double.Parse(retureVal.Split(',')[5]) / 1000);
                    modulationListL.F2F1 = (double.Parse(retureVal.Split(',')[7]));
                }
                else
                {
                    for (int i = 0; i < 3; i++)
            ...
            {
                retureVal = instrument.VisaQuery("XRESULT MI,HOPOFFM");
                if (retureVal.Contains("XMI"))
                {
                    avgPower = (double.Parse(retureVal.Split(',')[4]) / 1000)
               + double.Parse(item.FillValue);
                    modulationListM.F2 = (double.Parse(retureVal.Split(',')[5]) / 1000);
                    modulationListM.F2F1 = (double.Parse(retureVal.Split(',')[7]));
                }
                else
                {
                    for (int i = 0; i < 3; i++)
            ...
            {
                retureVal = instrument.VisaQuery("XRESULT MI,HOPOFFH");
                if (retureVal.Contains("XMI"))
                {
                    avgPower = (double.Parse(retureVal.Split(',')[4]) / 1000)
               + double.Parse(item.FillValue);
                    modulationListH.F2 = (double.Parse(retureVal.Split(',')[5]) / 1000);
                    modulationListH.F2F1 = (double.Parse(retureVal.Split(',')[7]));
                }
                else
                {
                    for (int i = 0; i < 3; i++)
        ...
        public TestData GetModulationindexF2Max(TestData item)
        {
            string retureVal = string.Empty;
            double avgPower = 0;
            if (item.Other.Split(':')[1] == data.Low_Freq)
            {
               avgPower =  modulationListL.F2;
            }
           else if (item.Other.Split(':')[1] == data.Mod_Freq)
            {
                avgPower = modulationListM.F2;
            }
           else if (item.Other.Split(':')[1] == data.Hi_Freq)
            {
                avgPower = modulationListH.F2;
            }
        ...
        public TestData GetModulationindexF2F1(TestData item)
        {
            string retureVal = string.Empty;
            double avgPower = 0;
            if (item.Other.Split(':')[1] == data.Low_Freq)
            {
                avgPower = modulationListL.F2F1;
            }
            else if (item.Other.Split(':')[1] == data.Mod_Freq)
            {
                avgPower = modulationListM.F2F1;
            }
            else if (item.Other.Split(':')[1] == data.Hi_Freq)
            {
                avgPower = modulationListH.F2F1;
            }
        ...
        public TestData GetSingleSensitivity_LostPack(TestData item)
        {
            //XSS,HOPOFFL,FALSE,0.016,3.361,FAIL,246,4,3,7405,252,249,7408
